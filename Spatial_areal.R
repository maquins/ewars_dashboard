observeEvent(c(input$population_spat,
  ...
  get_expected<-function(yr){
    for_exp<-dat_A[,c(exp_vars,"year","week")]  %>% 
      dplyr::filter(year==yr) %>% 
      dplyr::select(-year,-week)
    
    ca.ses<-ifelse(is.na(for_exp[,2]),0,for_exp[,2])
    Expected_cases<-expected(population=for_exp[,1],
                             cases=ca.ses,
                             n.strata =1)
    Expected_cases
  }
  
  #dat_A$Expected_cases<-foreach(a=sort(unique(dat_A$year)),.combine =c)%do% get_expected(a)
  ...
  ca.ses<-ifelse(is.na(for_exp[,2]),0,for_exp[,2])
  nstrata<-length(unique(dat_A$year))*length(unique(dat_A$week))
  ...
  observeEvent(c(input$Year_plot_spat,
                 input$Week_plot_spat),
               {
                 ...
                 output$plot3_spat<-renderLeaflet({
                   ...
                   pal <- colorBin("YlOrRd", domain =eval(parse(text=paste0("week_slice$`",yr_week,"`"))))
