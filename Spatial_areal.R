...
observeEvent(c(input$population_spat,
  ...
  nbh_a<-poly2nb(boundary_file_mod)
  ...
  dat_A<-var_names_spat()$dat %>% 
    ...
    dplyr::filter(district %in% boundary_file$district)
  ...
  dat_IDDS<-data.frame(district=boundary_file_mod$district) %>% 
    dplyr::mutate(id=1:n())
  ...
  list_mod_check<<-list(dat_A=dat_A,
                        boundary_file_mod=boundary_file_mod,
                        boundary_file=boundary_file_mod,
                        graph=H,
                        spat_adj=spat_adj)
